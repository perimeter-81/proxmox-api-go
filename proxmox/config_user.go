...
func (config ConfigUser) DeleteUser(ctx context.Context, client *Client) (err error) {
	...
	return client.Delete(ctx, "/access/users/"+config.User.ToString())
}
...
func (config *ConfigUser) UpdateUser(ctx context.Context, client *Client) (err error) {
	...
	err = client.Put(ctx, params, "/access/users/"+config.User.ToString())
	if err != nil {
		params, _ := json.Marshal(&params)
...
func NewConfigUserFromApi(ctx context.Context, userId UserID, client *Client) (*ConfigUser, error) {
	userConfig, err := client.GetItemConfigMapStringInterface(ctx, "/access/users/"+userId.ToString(), "user", "CONFIG")
	if err != nil {
		return nil, err
	}
...
func updateUser(ctx context.Context, user UserID, params map[string]interface{}, client *Client) error {
	return client.Put(ctx, params, "/access/users/"+user.ToString())
}
